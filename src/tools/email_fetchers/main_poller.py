...
from src.tools.email_fetchers.base_fetcher import AbstractEmailFetcher, EmailMessage
from src.tools.email_fetchers.imap_fetcher import ImapEmailFetcher
from src.tools.email_fetchers.gmail_api_fetcher import GmailAPIFetcher
from src.tools.email_fetchers.outlook_graph_fetcher import OutlookGraphAPIFetcher
    ...
    def _poll_account(self, account_config: Dict[str, Any]):
        ...
        while not self._stop_event.is_set():
            try:
                    ...
                    if new_emails:
                        ...
                        for email in new_emails:
                            try:
                                ...
                                logger.error(f"[{account_id}] Error processing email ID {email.id}: {e_proc}", exc_info=True)
                ...
                logger.error(f"[{account_id}] Connection error: {e_conn}. Will retry after interval.", exc_info=False) # exc_info=False as error is already logged by fetcher
    ...
    def start(self):
        ...
        for acc_conf in self.accounts_config.get("accounts", []):
            if acc_conf.get("enabled", False):
                thread = threading.Thread(target=self._poll_account, args=(acc_conf,), name=f"Poller-{acc_conf.get('id','Unnamed')}")
