...
class ImapEmailFetcher(AbstractEmailFetcher):
    ...
    def fetch_emails(self, criteria: Optional[Dict[str, Any]] = None) -> List[EmailMessage]:
        ...
        try:
            status, _ = self.imap.select(self.mailbox, readonly=(criteria is None or not criteria.get('mark_as_read_after_fetch', False))) # readonly if not marking as read here
            if status != 'OK':
