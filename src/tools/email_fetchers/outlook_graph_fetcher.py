...
class OutlookGraphAPIFetcher(AbstractEmailFetcher):
        ...
        if self.password:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"用户名/密码授权失败: {error_data.get('error')}: {error_data.get('error_description')}")
    ...
    def _make_graph_api_call(self, method, url_suffix, params=None, json_data=None, headers=None):
        ...
        try:
            ...
            logger.error(f"Outlook Graph API: HTTP 错误 {method} {full_url}: {e.response.status_code} - {e.response.text}")
